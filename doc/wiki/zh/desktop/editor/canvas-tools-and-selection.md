...
- 右键菜单文案现在通过 `en_US.json` / `zh_CN.json` 中的 `OCR Selected Regions`、`Translate Selected Regions`、`Copy Region`、`Paste Style`、`Delete Selected Regions`、`Add Text Box`、`Paste Region` 和 `Refresh View` 国际化；删除项会带上选中区域数量。仿制印章活动时右键被取样占用，菜单完全屏蔽。
