...
编辑器的大多数高频操作都可以用键盘或滚轮完成：撤销/重做、复制/粘贴/删除、切换文本方向、切换画布工具、切换图片，以及用滚轮组合调整画笔大小和选中区域字号。这里列出 `EditorShortcutManager` 实际注册的全部快捷键与滚轮组合，并解释焦点在文本控件、画布或浮动富文本窗口时这些按键分别交给谁处理。
...
- `Ctrl+Q` 导出、`V` 切换选中文本框的横排/竖排、`1`/`2`/`3` 切换图像编辑页、`Q`/`W`/`E` 切换当前页工具、`A`/`D` 切换图片是固定字面量，不随平台变化；页签上的数字仅通过悬浮提示显示。
...
以下表格是 `_setup_editor_shortcuts()` 实际注册的全部键盘快捷键。焦点在文本控件（`QTextEdit` 或 `QLineEdit`）时的行为与画布焦点不同；`V`/`1`/`2`/`3`/`Q`/`W`/`E`/`A`/`D` 在文本焦点下会作为普通字符转发给文本控件，不切换文本方向、页签、工具或图片。
...
| `V` | 输入字符 `v` | 以最后选中的文本框为基准，在横排与竖排之间切换；多选时统一应用到全部选区 |
| `1` | 输入字符 `1` | 切换到“蒙版”页 |
| `2` | 输入字符 `2` | 切换到“画笔”页 |
| `3` | 输入字符 `3` | 切换到“印章”页 |
...
flowchart TD
    ...
    C -->|是| D["Undo/Redo/Copy/Paste/SelectAll<br/>调用文本控件方法；V/1/2/3/Q/W/E/A/D 转发字符"]
    C -->|否| E["编辑器语义"]
    E --> E1["Undo/Redo → controller<br/>Copy → 复制最后选中区域<br/>Paste → 粘贴样式或区域<br/>SelectAll → 全选区域<br/>Delete → 删除选中区域"]
    E --> E2["V → 横排 / 竖排<br/>1/2/3 → 蒙版 / 画笔 / 印章页<br/>Q/W/E → 当前页从左到右三个工具<br/>A/D → 文件列表上一张 / 下一张"]
```

### 文本焦点时的按键转发 {#text-widget-forwarding}

焦点在文本控件时，`V`/`1`/`2`/`3`/`Q`/`W`/`E`/`A`/`D` 的处理会临时禁用对应快捷键，然后向文本控件合成发送 `KeyPress` 与 `KeyRelease` 事件（例如 `v`），发送完再恢复快捷键，避免递归触发。`Ctrl+Q` 导出不区分焦点：它直接调用 `EditorView.export_image()`，导出前先 `flush_pending_changes()` 冲刷浮动富文本编辑器中防抖期内的正文与注音内容。
...
- 浮动富文本窗口以 `WA_ShowWithoutActivating` 显示，选区变化不会调用 `focus_text()` 抢焦点，画布保留焦点，因此 `Delete`/`V`/`A`/`D`/`1`/`2`/`3`/`Q`/`W`/`E` 在浮窗出现后仍按画布语义工作；点击浮窗内的文本框后才进入文字编辑。
