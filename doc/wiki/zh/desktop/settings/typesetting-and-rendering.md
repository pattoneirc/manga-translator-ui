...
该开关仅在排版模式为智能气泡（`balloon_fill`）时生效。默认值：关闭（`false`）。

| 输入/状态 | 开启后的行为 |
| --- | --- |
| 不含显式 `[BR]`、`【BR】` 或 `<br>` | 按选中气泡蒙版的最大内接矩形调用一次现有自动断句器；字号仍由后续公共 `balloon_fill` 流程处理。 |
| 已含显式断行 | 完整保留已有断点，以文本框中心为锚点，并把字号搜索上限扩大到该气泡连通域的边界。 |
| 气泡蒙版缺失或未完整包裹 OCR 文本行 | 走保守回退，不依据不可靠的气泡几何重新断句。 |
| 后续气泡文字框与先前文字框重叠 | 把后续文字缩小到可用的最大不重叠字号。 |

字号搜索最终仍以完整气泡蒙版检查文字渲染框；最大内接矩形只用于无断点文本的断句范围。气泡蒙版不可靠时应保持关闭。
