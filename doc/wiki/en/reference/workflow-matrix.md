...
| Export Translation | Main image and readable template by default; existing project JSON required when the toggle is on | Detection, OCR, and translation by default; with the toggle on skips image loading, colorization, upscaling, detection, OCR, translation, mask, inpainting, rendering, and JSON write-back | Project JSON and translated sidecar by default; with the toggle on only `translations/<stem>_translated.<template-format>` and unchanged JSON | [Export Translation](../workflows/export-translation.md) |
| Export Original Text | Main image and readable template by default; existing project JSON required when the toggle is on | Detection and OCR by default; with the toggle on skips image loading, colorization, upscaling, detection, OCR, translation, mask, inpainting, rendering, and JSON write-back | Project JSON and original sidecar by default; with the toggle on only `originals/<stem>_original.<template-format>` and unchanged JSON | [Export Original Text](../workflows/export-original.md) |
...
| `cli.export_from_local_json` | Defaults to `false`; enabling it makes both text exports read local project JSON without image/OCR/API stages or JSON write-back; disabled keeps the legacy flow |
